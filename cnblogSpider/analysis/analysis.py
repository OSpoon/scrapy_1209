...
class Analycis:
    ...
    def __init__(self):
        ...
        self.zfdb = self.client.testdb
        self.zfdb.authenticate("test", "123456")
    ...
    def get_area_from_net(self):
        ...
        for my_a in my_as:
            if my_a.text == "东莞" or "周边" in my_a.text:  # 清除东莞地区的
                continue
            # print(my_a["href"])
            # print(my_a.text)
    ...
    def getAreaWeight(self):
        result = self.zfdb["东莞"].aggregate([{'$group': {'_id': '$region', 'weight': {'$sum': 1}}}])
        print(result)
        areaName = []
        areaWeight = []
        for item in result:
    ...
    # 获取户型数据（3 室 2 厅）
    def getRooms(self):
        results = self.zfdb["东莞"].aggregate([{'$group': {'_id': '$rooms', 'weight': {'$sum': 1}}}])
        roomList = []
        weightList = []
    ...
    # 获取租房面积
    def getAcreage(self):
        results0_30 = self.zfdb["东莞"].aggregate([
            {'$match': {'area': {'$gt': 0, '$lte': 30}}},
            {'$group': {'_id': '', 'count': {'$sum': 1}}}
        ])
        results30_60 = self.zfdb["东莞"].aggregate([
            {'$match': {'area': {'$gt': 30, '$lte': 60}}},
            {'$group': {'_id': '', 'count': {'$sum': 1}}}
        ])
        results60_90 = self.zfdb["东莞"].aggregate([
            {'$match': {'area': {'$gt': 60, '$lte': 90}}},
            {'$group': {'_id': '', 'count': {'$sum': 1}}}
        ])
        results90_120 = self.zfdb["东莞"].aggregate([
            {'$match': {'area': {'$gt': 90, '$lte': 120}}},
            {'$group': {'_id': '', 'count': {'$sum': 1}}}
        ])
        results120_200 = self.zfdb["东莞"].aggregate([
            {'$match': {'area': {'$gt': 120, '$lte': 200}}},
            {'$group': {'_id': '', 'count': {'$sum': 1}}}
        ])
        results200_300 = self.zfdb["东莞"].aggregate([
            {'$match': {'area': {'$gt': 200, '$lte': 300}}},
            {'$group': {'_id': '', 'count': {'$sum': 1}}}
        ])
        results300_400 = self.zfdb["东莞"].aggregate([
            {'$match': {'area': {'$gt': 300, '$lte': 400}}},
            {'$group': {'_id': '', 'count': {'$sum': 1}}}
        ])
        results400_10000 = self.zfdb["东莞"].aggregate([
    ...
    def showWorkCloud(self, content, image_filename, font_filename, out_filename):
        ...
        text = " ".join(tags)
        # 需要显示的背景图片
        img = plt.imread(path.join(d, image_filename))
        # 指定中文字体, 不然会乱码的
